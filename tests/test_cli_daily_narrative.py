...
def test_daily_narrative_smoke():
    """Smoke test to ensure daily command with --narrative doesn't crash."""
    with patch("engine.cli.utils.get_adapter") as mock_get_adapter:
        ...
        # Run command
        result = runner.invoke(app, ["daily", "br/mega-sena", "--narrative", "--no-quiet"])
        
        assert result.exit_code == 0
        assert "Daily Intelligence Narrative" in result.stdout
